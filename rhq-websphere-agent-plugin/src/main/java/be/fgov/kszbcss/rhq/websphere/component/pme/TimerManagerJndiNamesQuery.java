...
public class TimerManagerJndiNamesQuery implements ConfigQuery<String[]> {
    private static final long serialVersionUID = 1L;
    
    private static final Logger log = LoggerFactory.getLogger(TimerManagerJndiNamesQuery.class);
    
    private final String node;
    private final String server;
    
    public TimerManagerJndiNamesQuery(String node, String server) {
        this.node = node;
        this.server = server;
    }
    ...
    public String[] execute(Config config) throws JMException, ConnectorException, InterruptedException, ConfigQueryException {
        ...
        for (TimerManagerInfoCO tm : config.allScopes(node, server).path(TimerManagerProviderCO.class).path(TimerManagerInfoCO.class).resolve(false)) {
            result.add(tm.getJndiName());
        }
        if (log.isDebugEnabled()) {
            log.debug("Loaded timer managers for node '" + node + "' and server '" + server + "': " + result);
        }
    ...
    public boolean equals(Object obj) {
        if (obj instanceof TimerManagerJndiNamesQuery) {
            TimerManagerJndiNamesQuery other = (TimerManagerJndiNamesQuery)obj;
